...
cll_node *head, *tail, *prev_node, *next_node, *current_node, *temp, *new_node;

struct node *create_CLL(int no_of_elements)
{
    cll_node data_user;
    int i = 0;
    if ((head == NULL))
    {
        ...
        while (i < no_of_elements)
        {
            ...
            if (head == NULL)
            {
                ...
                current_node = (cll_node *)malloc(sizeof(cll_node));
                current_node->data = data_user.data;
                current_node->id = data_user.id;
                current_node->next = current_node; // Here the Circulation Happens
            ...
            else
            {
                ...
                int i = 0;
                while (prev_node->next != (tail->next)) // The End of Circular List instead of NULL :D
                {
                ...
                new_node->next = head;
                tail = new_node; // End Of List With Tail To differ between begining and end ^^
            }
        }
        if (i == no_of_elements)
        {
...
void display_cll() // Display All Elements of The list
{
    int i = 0;
    current_node = head;
...
int insert_node() // Insert any node at any specific required position
{
    ...
    printf("Enter Position For Adding Data!\n");
    scanf("%d", &pos);
    printf("--------------------------------------------------\n");
    ...
    else if ((head == NULL) && (pos > 1)) // For Testing Only.. It wouldn't enter here because there is a loop in the begining of the func
    {                                     //                    to let user enter the correct position :>
        printf("The Position Entered exceedded the list length!\n");
        printf("Please, Enter a positive integer position\n");
        printf("--------------------------------------------------\n");
        return Fail;
    ...
    else
    {
        ...
        else
        {
            ...
            */
            ...
            next_node = prev_node->next; // Next Node After The Required Position
            prev_node->next = new_node;
...
int delete_node()
{
    ...
    if (list_length != 0)
    {
        printf("Enter Position For Deleting Data Stored!\n");
        scanf("%d", &pos);
        printf("--------------------------------------------------\n");
...
void read_or_change_data_by_pos(int choice)
{
    ...
    else
    {
        if (choice == Choice_Read)
        {
            printf("List is Empty, There is nothing to Read from!");
            printf("--------------------------------------------------\n");
        }
        else if (choice == Choice_Change)
        {
            printf("List is Empty, There is nothing to Change Data from!\n");
            printf("--------------------------------------------------\n");
        }
    }
}
...
{
    int id, data_change, list_length;
    list_length = get_length_list();
    if (list_length != 0)
    {
        if (choice == Choice_Read)
        ...
        else
        {
            ...
            else
            {
                ...
                if (current_node->next == (tail->next))
                {
                    if (id == current_node->id)
                    {
                        if (choice == Choice_Read)
...
int entry_number_of_elements()
{
    ...
    printf("-------------------------------------------\n");
    current_node = create_CLL(no_of_elements);
    // while (i < no_of_elements)
    // {
    //     printf("Enter Data of The Node!\n");
    //     scanf("%d", &user_data);
